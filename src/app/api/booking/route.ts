...
import { getSupabaseAdmin, BOOKING_BUCKET } from "@/lib/supabase";
...
export async function POST(req: Request) {
  try {
    ...
    // Handle optional document upload → Supabase Storage bucket
    ...
    if (file && file instanceof File && file.size > 0) {
      ...
      if (supabase) {
        ...
        if (upErr) {
          ...
          // Signed URL (bucket is private) valid ~30 days for the owner email
    ...
    if (!supabase) {
      // Nothing configured yet — don't lie to the user.
      return NextResponse.json(
        {
          ok: true,
          note: "Received (backend not yet configured — set Supabase & Resend env vars).",
        },
        { status: 200 }
      );
    }
